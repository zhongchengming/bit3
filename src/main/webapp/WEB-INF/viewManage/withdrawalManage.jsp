    ...
    <form id="Formdata">
        <div class="modal-dialog" id="modal_dialog">
            <div class="modal-content">
                ...
                <input type="hidden" name="urlType" id="urlType" value="web">
...
<script>
            ...
            columns: [
                ...
                {title: '提现状态',
                    ...
                    formatter: function (value, row, index) {
                        if(row.state=='0'){
                            return  '<a id="edit" style="color:orange"> 审核中 </a>'
                        }else {
                            return "提现成功"
                        }
    ...
    window.operateEvents = {
        'click #edit': function (e, value, row, index) {
            parent.layer.confirm('确定已给对方转账？', {
                btn: ['确定','取消'], //按钮
                shade: false //不显示遮罩
            }, function(){
                $.ajax({
                    url:"user/withdrawalFinish",
                    type:"post",//请求方式
                    dataType : "json",
                    data:{"state":"1","id":row.id,"urlType":"web"},
                    success:function(data){//成功后执行方法；处理返回值
                        if(data.resultCode=="1"){
                            parent.layer.msg('操作成功', {icon: 1});

                            $table.bootstrapTable('refresh',{url:'user/withdrawalHistroy'});
                        }else{
        ...
        'click #deleted': function (e, value, row, index) {
            parent.layer.confirm('您确定不通过吗？', {
                ...
                $.ajax({
                    ...
                    type:"post",//请求方式
                    dataType : "json",
                    data:{"orderstate":"4","id":row.id,"urlType":"web"},
                    success:function(data){//成功后执行方法；处理返回值
                        if(data.resultCode=="1"){
                            $('#table').bootstrapTable('destroy');
                             parent.layer.msg('提交成功', {icon: 1});
                             $table('refresh',{url:'web/orderList'});
                        }else{
                            alert(data.resultMsg)
                        }
                    },
